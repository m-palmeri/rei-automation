...
def already_processed(page_id: str, edit_ts_iso: str) -> bool:
    ...
    with psycopg.connect(DB_URL, autocommit=True) as conn, conn.cursor() as cur:
        ...
        cur.execute("""
            ...
            VALUES (%s, %s)
            ON CONFLICT (page_id)
            DO UPDATE SET last_seen_edit = GREATEST(page_state.last_seen_edit, EXCLUDED.last_seen_edit),
                          updated_at = NOW()
        """, (page_id, edit_dt))
    return processed
...
def mark_processed(page_id: str, edit_ts_iso: str) -> None:
    ...
    with psycopg.connect(DB_URL, autocommit=True) as conn, conn.cursor() as cur:
        cur.execute("""
            ...
            VALUES (%s, %s, %s)
            ON CONFLICT (page_id)
            DO UPDATE SET last_processed_edit = GREATEST(page_state.last_processed_edit, EXCLUDED.last_processed_edit),
                          last_seen_edit = GREATEST(page_state.last_seen_edit, EXCLUDED.last_seen_edit),
                          updated_at = NOW()
...
def dlq_put(page_id: str, edit_ts_iso: str, error: str) -> None:
    edit_dt = _parse_iso_z(edit_ts_iso)
    with psycopg.connect(DB_URL, autocommit=True) as conn, conn.cursor() as cur:
        cur.execute("INSERT INTO dlq (page_id, edit_ts, error) VALUES (%s, %s, %s)", (page_id, edit_dt, error[:8000]))